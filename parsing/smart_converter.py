...
#
# @usage python smart_converter.py [options]
#   -a
#   -c <column list> [...]
#   -f <input_file>
#   -m <method>
...
class DDFWParser:
    all_normal_cols = ["instance", "ddfw_version", "a", "A", "c", "C", "w",
                "cpu_timeout", "flip_timeout", "method", "seed",
    ...
    all_weight_cols = ["instance", "ddfw_version", "a", "A", "c", "C", "w",
            "cpu_timeout", "flip_timeout", "method", "seed", "flips",
            "transfers", "unsat_weight", "unsat_count",
    ...
    def parse_line(self, line, outfile):
...
if options.infile is not None and options.directory is not None:
    ...
    if os.path.isdir(options.directory):
        _, _, files = next(walk(options.directory))
        input_files = list(map(lambda x: options.directory + "/" + x, files))
    else:
        print("Supplied directory is not a directory", file=sys.stderr)
        exit()
...
for f in input_files:
    ...
    for line in inlines:
        parser.parse_line(line, outfile)
